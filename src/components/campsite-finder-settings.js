...
import { Form, Checkbox, Card, List } from 'semantic-ui-react'
...
const CampsiteFinderSettingsForm = ({
  ...
  return (
    ...
    >
      ...
      <Card.Meta className='campsite-finder-card__sub-heading'>Dates</Card.Meta>
      <Form.Group inline>
        <Form.Radio
          label='Next 6 Months'
          name='datesSelector'
      ...
      {dateOption === SPECIFIC_DATES &&
        <DateRangePicker
          ...
          onFocusChange={focusedDate => handleDateFocusChange(_id, focusedDate)}
          numberOfMonths={1}
          isOutsideRange={date => {
          ...
          {emailAddresses.map((emailAddress, i) => {
            return (
              <List.Item
                ...
                onClick={(e, { children }) =>
                  handleUpdateCampsiteFinder(_id, {
                    emailAddresses: JSON.stringify(remove(i, 1, emailAddresses))
                  })}
              >
